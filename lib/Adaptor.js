    ...
    Object.keys(this.path2control).forEach(function(path){
        this.path2control[path].on('state_update', function(control){
            if (that.controlid2path[control.id] !== undefined){
                var aState = JSON.stringify(control.get_state());
                var stateTopic = that.controlid2path[control.id]+'/state'
                stateTopic = stateTopic.replace("+", "_").replace("#", "_")
                that.emit('for_mqtt', stateTopic, aState);
                // also emit raw state for further processing
                var raw = {
                    ...
                    metadata: {
                        ...
                        room : that.room2name[control.room],
                        categoryUuid : control.category,
                        category : that.cat2name[control.category],
                        stateTopic: stateTopic,
                        commandTopic: stateTopic.replace(/\/state$/, '/cmd')
                    }
                }
                var rawPayload = JSON.stringify(raw/*, null, 2*/);
                that.emit('for_mqtt', that.mqtt_prefix + '/raw/state', rawPayload);
            }else{
